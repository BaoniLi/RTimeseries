...
#设置工作空间
setwd("D:/202111/NH/YRD/TE")


#读取站点数据及待插值数据
gauge_data_all<-na.omit(read.table("D:/202111/NH/YRD/te.txt"))
date_len = ncol(gauge_data_all) #获取总列数,用做循环条件
yzsta <- read.table("D:/202111/NH/YZstation/yrd1.txt",header=T)
...
for (i in seq(1,date_len,1)) {
  ...
  #控制插值分辨率,插入栅格模板
  template_raster<-raster("D:/202111/NH/YRD/krigtemp.tif")
  #bound_raster<-rasterize(bound,template_raster)
  
  v<- variogram(log(gauge_data) ~ 1, data =dsp1)
